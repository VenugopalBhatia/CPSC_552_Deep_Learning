...
class Discriminator(nn.Module):
    ...
    def forward(self, x, labels): # labels to be used in 5.4.
        le = self.label_embedding(labels)
        x = torch.cat([x,le], 1)
        output = self.hidden_layer1(x)
        output = self.hidden_layer2(output)
...
discriminator = Discriminator(794,1).to(device) # initialize both models, and load them to the GPU or CPU.
generator = Generator(110,784).to(device)
...
def train_generator(batch_size):
    ...
    generator_loss = loss_f(output, labels)
...
def train_discriminator(batch_size, images, labels=None): # labels to be used in 5.4.
    ...
    labels_r = torch.ones(batch_size, 1).to(device)
    labels_f = torch.zeros(batch_size, 1).to(device)
    ...
    fake_labels = torch.randint(0, 10, (batch_size,)).to(device)
    images_f = generator(noise,fake_labels)
    outputs_f = discriminator(images_f,fake_labels)
    loss_fcn_f = loss_f(outputs_f,labels_f)

    # print(images.shape)
    # print(images_f.shape)

    outputs_r = discriminator(images,labels)
    loss_fcn_r = loss_f(outputs_r,labels_r)

    discriminator_loss = loss_fcn_f + loss_fcn_r
...
for epoch in tqdm(range(training_parameters['n_epochs'])):
    ...
    D_loss = []
    for batch, (imgs, labels) in tqdm(enumerate(train_loader)):
        ...
        #lossG = train_generator(batch_size) To train the generator twice
        G_loss.append(lossG)
        ...
        if ((batch + 1) % 500 == 0 and (epoch + 1) % 1 == 0):
            ...
            with torch.no_grad():  # disables gradient computation to speed things up
                ...
                for i, x in enumerate(generated_data):
                    ax[math.floor(i / batch_sqrt)][i % batch_sqrt].set_title(label_descriptions[int(fake_labels[i].item())]) # TODO: In 5.4 you can uncomment this line to add labels to images.
