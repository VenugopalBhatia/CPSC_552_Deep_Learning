...
def train(error_type = "MSE", regularization = None,lambda_ = 0,p = None):

    model = FCNN(num_input, num_classes,p = p)
    ...
    for ep in range(num_epochs):
        ...
        for batch_indx, batch in enumerate(trainloader):
            ...
            if regularization == "L1" :
                regularization_term = lambda_*sum(p.pow(1.0).sum() for p in model.parameters())

            if regularization == "L2" :
                regularization_term = lambda_*sum(p.pow(2.0).sum() for p in model.parameters())

           
            # print(pred.shape)
            # print(labels.shape)
            
            loss = loss_f(pred,labels) + regularization_term
